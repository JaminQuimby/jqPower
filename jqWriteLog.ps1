...
[Parameter()][IO.FileInfo] $Path = ”$env:temp\PowerShellLog.txt”,
[Parameter()][Switch]$Clobber,
[Parameter()][String]$EventLogName,
[Parameter()][String]$EventSource,
[Parameter()][Int32]$EventID = 1)
Begin{}
Process{
 try {
 $msg='{0}{1}:{2}:{3}' -f(" " * $Indent),(Get-Date -Format “yyyy-MM-dd HH:mm:ss”),$Level.ToUpper(),$Message;
   ...
   if($Clobber){$msg | Out-File -FilePath $Path -Force;}
     else{$msg | Out-File -FilePath $Path -Append;}
   if($EventLogName){
     if(-not $EventSource){$EventSource =([IO.FileInfo] $MyInvocation.ScriptName).Name;}
...
.EXAMPLE
PS C:\> Write-Log -Message "Your Message Here" -Path C:\LogFileName.log -Clobber -EventLogName 'Application'
.EXAMPLE
PS C:\> Write-Log -Message "Your Message Here" -Level Error -EventID 3 -Indent 2 -EventLogName 'Application' -EventSource "My Script"
.INPUTS
System.String
.OUTPUTS
No output.
.LINK
http://jaminquimby.com/joomla253/programing/96-powershell/
#>
    }